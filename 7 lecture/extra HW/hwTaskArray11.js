let array = [1,4,32,17,3,4,5,6,8,4,4,4,99,100];
let arraySecond = [0,0,2,3,3,4,5,6,3,8,2,1,1,6,6,8,8,8,4,4,4,99,100]; //[1,4,32,17,3,4,5,6,8,4,4,4,99,100]

function arrayDuplicatedValues (a,b) {
    let results = "";
    let longestLength = 0;
    let lesserLength = 0;
    let flag = true;
...
//Сравнявам дължините за да зная коя дължина къде да отиде :D    
    ...
    for (let i=0; i<longestLength; i++) {
        ...
        if (flag === true && i < lesserLength) {
            secondCopy[i] = b[i];
        } else if (flag === false && i < lesserLength){
            secondCopy[i] = a[i];
        }
    }
    //Тук правя копия на 2та масива. 

    for (let i=0; i<lesserLength; i++) {  
        
        for (let counter = 0; counter<longestLength; counter++) {
            if (secondCopy[i] === arrayCopy[counter] && arrayCopy[counter]!=="Nope") {
                helper = arrayCopy[counter];
                results += `${secondCopy[i]}\n`;
...
arrayDuplicatedValues(array,arraySecond);
